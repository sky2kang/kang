...
NOTE: matplotlib 는 선택적 의존성이다. 32비트 키움 환경에는 matplotlib
휠이 없을 수 있으므로, import 는 plot() 호출 시점까지 지연한다. 핵심
프로그램(hts.py/main.py)과 백테스트 텍스트 리포트는 matplotlib 없이도 동작한다.
"""
import os
...
def _load_matplotlib():
    """matplotlib 지연 로드 + 한글 폰트 설정. 미설치 시 안내 메시지와 함께 예외."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        ...
        raise RuntimeError(
            "백테스트 차트 저장에는 matplotlib 이 필요합니다. "
            "32비트 키움 환경에서는 휠이 없을 수 있습니다. "
            "텍스트 리포트는 차트 없이도 출력됩니다. "
